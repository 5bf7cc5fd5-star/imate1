...
def notify_admin_new_deposit(user: dict, dep: dict) -> dict:
    """SMS admin when a member submits a deposit + TxID."""
    name = user.get("name") or "Member"
    phone = user.get("phone") or "—"
    email = user.get("email") or "—"
    amount = dep.get("amount") or 0
    method = dep.get("method") or dep.get("network") or "—"
    txid = dep.get("reference") or dep.get("provider_ref") or dep.get("txId") or "—"
    msg = (
        f"Own Club DEPOSIT\n"
        f"{name} ({phone})\n"
        ...
        f"Open admin to approve."
    )
    # Support comma-separated notify list
    targets = [x.strip() for x in str(SMS_NOTIFY_TO or ADMIN_PHONE).split(",") if x.strip()]
    results = []
    for t in targets:
        results.append(send_sms(t, msg))
...
class Handler(BaseHTTPRequestHandler):
    ...
    def _deposit(self, user, body):
        ...
        try:
            sms_result = notify_admin_new_deposit(user, dep)
        except Exception as e:
